...
def test_grade_c_routes_are_serial_hash_bound_and_gap_preserving(monkeypatch, tmp_path):
    provisional = _provisional_module()
    assembled = importlib.import_module("gpm.qa.m25c_assembled")
    coverage = {"coverage": [
        {
            "region_id": region, "layer": "geometry", "grade": "A",
            "source_ids": [f"source-{region}"], "assertion_ids": [f"seam-{region}"],
            "evidence_summary": "reviewed Grade A packet claim",
            "exclusions": [], "known_gaps": [],
        }
        for region in provisional.GRADE_C_ROUTES_BY_REGION
    ]}

    changes = provisional._apply_grade_c_routes(coverage)
    for row in coverage["coverage"]:
        assert row["grade"] == "C"
        assert row["known_gaps"] == list(assembled.GRADE_C_GAPS_BY_REGION[row["region_id"]])
    assert [change["change_id"] for change in changes] == list(assembled.GRADE_C_CHANGE_IDS)
    routes = [
        route for region_routes in provisional.GRADE_C_ROUTES_BY_REGION.values()
        for route in region_routes
    ]
    for change, route in zip(changes, routes, strict=True):
    ...
    with pytest.raises(SystemExit, match="review decisions drifted"):
        provisional._apply_grade_c_routes({"coverage": []})


def test_assembled_coverage_allows_only_exact_grade_c_exceptions():
    assembled = importlib.import_module("gpm.qa.m25c_assembled")
    rows = [
    ...
    exceptions = []
    for region, gaps in assembled.GRADE_C_GAPS_BY_REGION.items():
        exception = next(
            row for row in rows if (row["region_id"], row["layer"]) == (region, "geometry")
        )
        exception["grade"] = "C"
        exception["known_gaps"] = list(gaps)
        exceptions.append(exception)
    changelog = {"changes": [
        {"change_id": change_id} for change_id in assembled.GRADE_C_CHANGE_IDS
    ]}
    coverage = {"coverage": rows, "known_gaps": [], "exclusions": []}

    assembled._qualify_assembled_coverage(coverage, changelog)
    exceptions[-1]["known_gaps"] = exceptions[-1]["known_gaps"][:-1]
