...
| M25C | in progress; spatial phase 1 implemented; assembled candidate remains ordinary-QA blocked | shared final-artifact qualifier, transactional `assembled-pass` generation, covered-zero seam observability, revision-bound border-applicability candidates, complete render, ordinary pending-review QA | byte-identical duplicate assembly and 30-sheet render; 58 non-review blockers plus one warning remain after exact nine-record Southern Europe corridor reconstruction | `1.0.0-assembled.1` certification-review candidate; all review/certification/runtime/public permissions false | separately review every remaining corridor/border packet and the five applicability audits; only zero-error preflight may enable independent review |
