...
| M25C | in progress; assembled and ordinary-QA blocked | shared final-artifact qualifier, transactional `assembled-pass` generation, complete render, ordinary pending-review QA | 54 non-review blockers reproduced; evidence-first spatial re-research completed with no fabricated-border recommendation | `1.0.0-assembled.1` certification-review candidate; all review/certification/runtime/public permissions false | reviewer decision on covered executable-zero semantics, Southern Europe repair, border applicability, and conditional regional digitization; only zero-error preflight may enable independent review |
