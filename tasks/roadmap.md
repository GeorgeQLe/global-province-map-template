...
| M25C | in progress; spatial bundle approved with amendments; assembled candidate remains ordinary-QA blocked | shared final-artifact qualifier, transactional `assembled-pass` generation, complete render, ordinary pending-review QA | 54 non-review blockers reproduced; five-part evidence-first spatial decision approved with stricter observability, provenance, applicability, conditional-digitization, and per-corridor review gates | `1.0.0-assembled.1` certification-review candidate; all review/certification/runtime/public permissions false | implement only the approved first remediation phase, then stop at separate evidence gates for every remaining corridor or border; only zero-error preflight may enable independent review |
