...
| M25C | in progress; 22 of 22 regions compositionally migrated | compositional seam QA and nine Natural Earth 5.1.1 modern controls now supplement the five-facet/typed-relationship model; runtime and publication guards remain unchanged | Asia/Europe research specifies six more 75 km / 0.20 controls: five measure 0.7985-1.0 and Southern Europe exposes a zero-transition status collapse that must fail closed | all six surviving Asia/Europe borders are circular generated-edge self-comparisons and are recommended for retirement alongside the nine open Americas/Africa positive gates | await reviewer decision on `tasks/m25c-asia-europe-assertion-reresearch.md`, then remediate without weakening the seven existing failures |
